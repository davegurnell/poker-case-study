...
case class Hand(cards: Vector[Card]) {
  ...
  def chooseCard: Vector[Selection[Card]] =
    cards.map(card => Selection(card, this - card))
  ...
  def choosePair: Vector[Selection[(Card, Card)]] = {
    ...
    for {
      ...
      Selection(b, rest) <- rest.chooseCard if a.value == b.value && a.suit < b.suit
  ...
  def threeOfAKind: Vector[Selection[(Card, Card, Card)]] = {
    ...
    for {
      ...
      Selection(c, rest)      <- rest.chooseCard if b.value == c.value && b.suit < c.suit
  ...
  def fourOfAKind: Vector[Selection[(Card, Card, Card, Card)]] = {
    ...
    for {
      ...
      Selection(d, rest)         <- rest.chooseCard if c.value == d.value && c.suit < d.suit
